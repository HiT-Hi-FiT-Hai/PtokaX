...
void BanDialog::DoModal(HWND hWndParent, BanItem * pBan/* = NULL*/) {
    ...
    ::SetWindowLongPtr(m_hWnd, GWLP_WNDPROC, (LONG_PTR)StaticBanDialogProc);
...
bool BanDialog::OnAccept() {
	...
	if(bNickBan == false && bIpBan == false) {
		::MessageBox(m_hWnd, LanguageManager->sTexts[LAN_YOU_CANT_CREATE_BAN_WITHOUT_NICK_OR_IP], sTitle.c_str(), MB_OK | MB_ICONEXCLAMATION);
		return false;
	}
	...
	if(bNickBan == true && iNickLen == 0) {
		::MessageBox(m_hWnd, LanguageManager->sTexts[LAN_FOR_NICK_BAN_SPECIFY_NICK], sTitle.c_str(), MB_OK | MB_ICONEXCLAMATION);
		return false;
	}
	...
	if(bIpBan == true) {
		if(iIpLen == 0) {
			::MessageBox(m_hWnd, LanguageManager->sTexts[LAN_FOR_IP_BAN_SPECIFY_IP], sTitle.c_str(), MB_OK | MB_ICONEXCLAMATION);
			return false;
		} else if(isIP(sIP, iIpLen) == false) {
			::MessageBox(m_hWnd, (string(sIP) + " " + LanguageManager->sTexts[LAN_IS_NOT_VALID_IP_ADDRESS]).c_str(), sTitle.c_str(), MB_OK | MB_ICONEXCLAMATION);
			return false;
        }
	}
	...
	if(bTempBan == true) {
        ...
        if(::SendMessage(hWndWindowItems[DT_TEMP_BAN_EXPIRE_DATE], DTM_GETSYSTEMTIME, 0, (LPARAM)&stDate) != GDT_VALID ||
            ...
            ::MessageBox(m_hWnd, LanguageManager->sTexts[LAN_BAD_TIME_SPECIFIED], sTitle.c_str(), MB_OK | MB_ICONEXCLAMATION);
		...
		if(ban_time <= acc_time || ban_time == (time_t)-1) {
			::MessageBox(m_hWnd, LanguageManager->sTexts[LAN_BAD_TIME_SPECIFIED_BAN_EXPIRED], sTitle.c_str(), MB_OK | MB_ICONEXCLAMATION);
								...
								if(((nxtBan->ui8Bits & hashBanMan::IP) == hashBanMan::IP) == true) {
                                    if(pBan->ui32IpHash == nxtBan->ui32IpHash) {
										if(((pBan->ui8Bits & hashBanMan::FULL) == hashBanMan::FULL) == false) {
                                            ...
                                            ::MessageBox(m_hWnd, LanguageManager->sTexts[LAN_SIMILAR_BAN_EXIST], sTitle.c_str(), MB_OK | MB_ICONEXCLAMATION);
                                            return false;
                                        } else {
...
void BanDialog::BanDeleted(BanItem * pBan) {
    ...
    ::MessageBox(m_hWnd, LanguageManager->sTexts[LAN_BAN_DELETED_ACCEPT_TO_NEW], sTitle.c_str(), MB_OK | MB_ICONEXCLAMATION);
}
//------------------------------------------------------------------------------
